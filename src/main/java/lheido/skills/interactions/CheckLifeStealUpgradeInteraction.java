...
/**
 * Interaction pour vérifier les prérequis d'upgrade du skill LifeSteal.
 * Paramétrable via JSON avec RequiredLevel et TargetLevel.
 *
 * - RequiredLevel: Le niveau minimum requis (0 = aucun prérequis, pour LifeSteal A)
 ...
 */
public class CheckLifeStealUpgradeInteraction
    extends SimpleInstantInteraction
{

    public static final BuilderCodec<
        CheckLifeStealUpgradeInteraction
    > CODEC = BuilderCodec.builder(
        CheckLifeStealUpgradeInteraction.class,
        CheckLifeStealUpgradeInteraction::new,
    ...
    protected void firstRun(
        ...
        // Récupérer le component LifeSteal existant (peut être null)
        LifeStealSkillComponent existingComponent =
        ...
        if (currentLevel >= targetLevel) {
            String message =
                targetLevel == 4
                    ? "You already have maximum life steal!"
                    : "You already have Life Steal level " +
                      targetLevel +
                      " or higher!";
            player.sendMessage(Message.raw(message));
            interactionContext.getState().state = InteractionState.Failed;
